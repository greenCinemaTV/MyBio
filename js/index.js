...
window.addEventListener("scroll", function () {
  ...
  const pageData = async function (title) {
    ...
    const fetchedData = await fetch("./incaseApi/db.json");
  ...
  async function updateMenuVisibility() {
    ...
    menuItems.forEach((menuItem) => {
      ...
      const section = menuItem.className.split(' ')[0];
  
      // Check if the section is supposed to be visible or hidden
      if (data[section].showonPage) {
  ...
  async function createAboutSection() {
    try {
      ...
      aboutTitle.textContent = about.title || "About Me"; // Default title if none provided
  
      const aboutContent = document.createElement("div");
      aboutContent.innerHTML = `
        Welcome to 
        <a style="text-decoration: none;" href="https://www.youtube.com/@GreenCinemaTV" target="_blank" rel="noopener noreferrer">
          <strong>GreenCinema TV!</strong>
        </a> 
        ${about.description || "Explore our amazing content!"}`; // Fallback description
      ...
      document.querySelector("#about").innerHTML = "<p>Unable to load about information.</p>";
    }
  }
  ...
  async function createContentSection() {
    try {
      // Fetch content data and video data
      ...
      document.querySelector("#content h2").textContent = content.title || "My Top Content"; // Default title if missing
      ...
      videos.forEach((video) => {
        ...
        img.src = video.imgUrl || "default-thumbnail.jpg"; // Fallback image if none provided
        img.alt = video.title ? `${video.title} Thumbnail` : "Video Thumbnail"; // Dynamic alt text
        ...
        titleText.innerHTML = `Title: <b title="${video.title}">${video.title.slice(0, 15)}...</b>`;
        titleText.title = video.title;
  
        const typeText = document.createElement("p");
        typeText.innerHTML = `${video.type.length > 10 ? "Description:" : "Type:"} <b>${video.type.slice(0, 11)}...</b>`;
      ...
      document.querySelector("#content").innerHTML = "<p>Unable to load content information.</p>";
    }
  }
  ...
  async function createContactSection() {
    try {
      ...
      document.querySelector("#contact h2").textContent = contact.title || "Contact Me"; // Default title if missing
      document.querySelector("#contact p").textContent = contact.description || "Get in touch!"; // Default description
  
      // Select the contact container
      const contactContainer = document.querySelector("#contact .contact_info ul");
      if(!contact.showonPage) {
      ...
      contact.contactInfo.forEach((info) => {
        if (!info.actions) return; // Skip if actions are not allowed
      ...
      // Optionally display an error message to the user
      document.querySelector("#contact").innerHTML = "<p>Unable to load contact information.</p>";
    }
  }
